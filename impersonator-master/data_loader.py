...
class Rescale(object):
    ...
    def __call__(self, sample):
        imidx, image, lbl = sample['imidx'], sample['image'], sample['label']
    ...
    def __call__(self, sample):
        ...
        if self.flag == 2:  # with rgb and Lab colors
            ...
            if image.shape[2] == 1:
                ...
                tmpImg[:, :, 1] = (image[:, :, 1]-0.456)/0.224
                tmpImg[:, :, 2] = (image[:, :, 2]-0.406)/0.225
            # tmpImg[:, :, 0] = (image[:, :, 0]-np.mean(image[:, :, 0])
            #                    )/np.std(image[:, :, 0])
            # tmpImg[:, :, 1] = (image[:, :, 1]-np.mean(image[:, :, 1])
            #                    )/np.std(image[:, :, 1])
            # tmpImg[:, :, 2] = (image[:, :, 2]-np.mean(image[:, :, 2])
            #                    )/np.std(image[:, :, 2])
            # tmpImg = image
