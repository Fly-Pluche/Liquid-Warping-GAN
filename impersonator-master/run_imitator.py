...
if __name__ == "__main__":
    ...
    print('test_opt.src_path',test_opt.src_path)
    imitator.personalize(test_opt.src_path, visualizer=visualizer)
    print('\n\t\t\tPersonalization: completed...')

    if test_opt.save_res:
        pred_output_dir = mkdir(os.path.join(test_opt.output_dir+ 'imitators_origin'))
    ...
    outputs=imitator.inference(tgt_paths, tgt_smpls=None, cam_strategy='smooth',
                       output_dir=pred_output_dir, visualizer=visualizer, verbose=True)

    """合成视频"""
    img_path_list = sorted(glob.glob('%s/*.jpg' % pred_output_dir))
    path='/home/ray/workspace/Fly_Pluche/impersonator-master/outputs/results/demos'
    make_video(path, img_path_list, save_frames_dir=None, fps=30)
